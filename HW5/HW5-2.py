...
def gridsearch(kernel, x, y, x_test, y_test):
    ...
    if(kernel=='linear'):
        ...
        for i in range(len(cost)):
            ...
            #set svm parameter
            param = svm_parameter(parameter)
            #train a svm model and use cross-validation
            acc = svm_train(prob, param)
            # if accuracy is larger than old acc, save new acc and parameter
            if(acc>best_acc):
                best_acc = acc
                best_cost = cost[i]
        # test the model with best parameter which is found with cross-validation
        ...
        # save the best parameter and best cross-validation accuracy
...
def user_define_kernel(gamma, x1, x2, dataset_row):
    # linear kernel is x1 * x2.T
    linear = x1.dot(x2.T)
    # RBF kernel is e ^ (-gamma * ||x1-x2||^2)
    RBF = np.exp(-gamma * cdist(x1, x2, 'sqeuclidean'))
    kernel = np.hstack((np.arange(1, dataset_row + 1).reshape(-1, 1), linear+RBF))
    return kernel
...
def gridsearch_user(x, y, x_test, y_test):
    ...
    for i in range(len(cost)):
        for j in range(len(gamma)):
            kernel_train = user_define_kernel(gamma[j], x, x, 5000)
            # use user-defined kernel to train and find the best parameter by cross-validation
            ...
            if(acc>best_acc):
                ...
                best_gamma = gamma[j]
    # test the model with best parameter which is found with cross-validation
    kernel_train = user_define_kernel(best_gamma, x, x, 5000)
    kernel_test = user_define_kernel(best_gamma, x_test, x, 2500)
